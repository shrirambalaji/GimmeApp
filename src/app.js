    ...
    switch (text[0]) {
        ...
        var Country;
            ...
            options = {};
                ...
                if (err) {
                    return {
                        text: "Could'nt fetch the forex data. Try Again Later."
                    }
                }
                var body = JSON.parse(body);
            ...
            case "dictionary":
            var word=text[1];
            var uri="http://api.pearson.com/v2/dictionaries/entries?headword=" +word;
            options={};
             request.get(uri, options, function(err, res, body) {
                if (err) {
                    return {
                ...
                flock.callMethod('chat.sendMessage', config.botToken, {
                        ...
                        "attachments": [{
                            "title": word,
                            "description": "Part of Speech :" +results[i].part_of_speech + "\n" +
                            "Definition : " + results[i].senses[0].definition + "\n" 
                       //     "Example" + results[i].senses[0].examples.text + "\n"
